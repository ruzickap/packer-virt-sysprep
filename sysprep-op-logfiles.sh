...
for logd in ${logd_locations[@]}; do
  if [ -d "${logd}" ]; then
    ...
    while [[ ${logd_path:0:1} = "/" ]] && [[ ${#logd_path} -gt 1 ]] && [[ ${on_tmpfs} = false ]]; do
      defifs=${IFS}
      IFS=$'\n' # Set for convenience with mount output
      for mountpoint in $(mount -l -t tmpfs | cut -d' ' -f3); do
